...
SlMqttClientLibCfg_t Mqtt_ClientCfg = { 0,
TASK_PRIORITY,
RCV_TIMEOUT,
true, (long (*)(const char *, ...)) UART_PRINT };

SlMqttServerCfg_t Mqtt_Server = { { 0,
NULL,
PORT_NUMBER, 0, 0, 0,
NULL },
LOOPBACK_PORT,
...
void MqttClientServer(void *pvParameters) {
	...
	for (;;) {
		...
		case PUB_PUSH_BUTTON_PRESSED:
			...
			//
			lRetVal = sl_ExtLib_MqttClientSend(BridgeHndl.ClientCtxHndl,
					(const char *) pub_topic, data, strlen((char*) data), QOS2,
					RETAIN);
		...
		case MSG_RECV_BY_SERVER:
			...
			lRetVal =
					sl_ExtLib_MqttClientSend(BridgeHndl.ClientCtxHndl,
							((const char *) (que_elem_recv.msg_ptr)
									+ uiTopOffset),
							((unsigned char*) (que_elem_recv.msg_ptr)
									+ uiPayloadOffset), msg_head.pay_len,
							msg_head.qos, msg_head.retain);
			free(que_elem_recv.msg_ptr);
			break;
		...
		case MSG_RECV_BY_CLIENT:
			...
			lRetVal =
					sl_ExtLib_MqttServerSend(
							...
							((unsigned char*) (que_elem_recv.msg_ptr)
									+ uiPayloadOffset), msg_head.pay_len,
							msg_head.qos, msg_head.retain, 0);
			free(que_elem_recv.msg_ptr);
			break;
